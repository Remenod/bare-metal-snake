...
static uint8_t peekb(unsigned seg, unsigned off)
{
    volatile uint8_t *addr = (volatile uint8_t *)(seg * 16 + off);
    return *addr;
}

static inline void pokew(unsigned seg, unsigned off, unsigned short val)
{
    volatile unsigned short *ptr = (volatile unsigned short *)(seg * 16 + off);
    *ptr = val;
}

static void _vmemwr(unsigned seg, unsigned off, uint8_t *src, unsigned count)
{
    uint8_t *dst = (uint8_t *)(seg + off);
...
static inline unsigned short peekw(void *addr)
{
...
//
//  0x5F, 0x4F, 0x50, 0x82, 0x55, 0x81, 0xBF, 0x1F,
//  0x00, 0x4F, 0x0D, 0x0E, 0x00, 0x00, 0x02, 0x9F,
...
//
//  0x5F, 0x4F, 0x50, 0x82, 0x54, 0x80, 0xBF, 0x1F,
//  0x00, 0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x1F,
...
static uint8_t g_8x16_font[256][FONT_HEIGHT] =
    {
...
static uint8_t g_320x200x256_palette[256][3] = {
    {0, 0, 0},
    {0, 0, 42},
    {0, 42, 0},
    {0, 42, 42},
...
static uint8_t g_80x25_text_palette[256][3] = {
    {0, 0, 0},
    {0, 0, 42},
    {0, 42, 0},
    {0, 42, 42},
...
static void vmemwr(unsigned dst_off, uint8_t *src, unsigned count)
{
...
// *****************************************************************************/
// static void write_font(uint8_t *buf, unsigned font_height)
// {
...
void write_font(uint8_t font[256][FONT_HEIGHT])
{
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        {
            // vram[i * 32 + j] = font[i][j];
            pokeb(0xB800, i * 32 + j, font[i][j]);
        }
        /* Якщо FONT_HEIGHT < 32 — заповнити залишок нулями (бажано) */
        for (int j = FONT_HEIGHT; j < 32; j++)
            vram[i * 32 + j] = 0;
    }

    /* Відновити регістри (знову ставимо індекси перед записом) */
    outb(VGA_SEQ_INDEX, 2);
    outb(VGA_SEQ_DATA, seq2);
    outb(VGA_SEQ_INDEX, 4);
    outb(VGA_SEQ_DATA, seq4);
...
void dac_write_color(uint8_t index, uint8_t r, uint8_t g, uint8_t b)
{
...
void dac_read_color(uint8_t index, uint8_t *r, uint8_t *g, uint8_t *b)
{
...
void set_text_mode(/*int hi_res*/)
{
    ...
    write_palette(g_80x25_text_palette);
    write_font(g_8x16_font);
...
void demo_graphics()
{
    ...
    write_palette(g_320x200x256_palette);
    // g_wd = 320;
    // g_ht = 200;
